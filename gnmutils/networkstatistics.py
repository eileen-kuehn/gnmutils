...
class NetworkStatistics(object):
    def __init__(self, tme=None, event_count=0, data_size=0, traffic_in_size=.0, traffic_out_size=.0,
                 background_traffic_in_size=.0, background_traffic_out_size=.0, workernode=None, run=None, interval=20):
        self._tme = tme
    ...
    def getRow(self):
        return "%s,%d,%.2f,%.2f,%.2f,%.2f,%.2f" % (
            str(self._tme), self._event_count, self._data_size/1024.0, self._traffic_in_size, self._traffic_out_size,
            self._background_traffic_in_size, self._background_traffic_out_size
        )

    def getHeader(self):
        return "tme,event_count,data_size,traffic_in_size,traffic_out_size,background_traffic_in_size," \
               "background_traffic_out_size"
